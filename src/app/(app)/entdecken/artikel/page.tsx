...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  return {
    title: "Artikel | Entdecken | LoclSpots",
    description:
      "Tipps & Stories für München ab 30 — von der LoclSpots Community.",
    alternates: {
      canonical: `/entdecken/artikel${qs}`,
    },
  };
}

export default async function EntdeckenArtikelPage({ searchParams }: Props) {
  const params = await searchParams;
  ...
  return (
    <div>
      <h2 className="mb-1 font-serif text-xl font-semibold text-forest">
        Artikel
      </h2>
      ...
      <div className="mb-6 flex gap-2 overflow-x-auto pb-3 scrollbar-hide">
        {CATEGORIES.map((cat) => (
          <Link
            ...
            href={
              cat
                ? `/entdecken/artikel?category=${encodeURIComponent(cat)}`
                : "/entdecken/artikel"
            }
            className={cn(
