...
export default async function EntdeckenArtikelSlugPage({ params }: Props) {
  const { slug } = await params;
  const article = await getArticleBySlug(slug);
  ...
  return (
    <div className="space-y-8">
      <Link
        ...
        className="inline-block text-sm font-medium text-sage transition-colors hover:text-forest"
      >
      ...
      {related.length > 0 && (
        <section className="border-t border-sage/12 pt-8">
